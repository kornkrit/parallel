...
<html>
	...
	<script type="text/javascript">
		...
		<article class="select" style="color:black">
		
		<?php
		$courseID = $_GET["id"];
		$sql = " SELECT * FROM answer where course_id = '$courseID'";	
		
		$sql = "SELECT  `question_id` , question.qa, answer, COUNT( answer ) 
FROM  `answer` JOIN question ON question.id = answer.question_id
WHERE  `course_id` =  '$courseID' GROUP BY  `question_id` ,  `answer`";
		...
		$answer = $result[4];
		$date = $result[5];
		echo $id." ".$student_id." ".$question_id." ".$course_id." ".$answer." ".$date."<br>";
		}	
		?>
